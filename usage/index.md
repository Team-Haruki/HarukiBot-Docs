...
+ 推荐群聊页面仅接受熟人申请。
+ HarukiBot NEO目前处于公测状态，可能存在各种各样的Bug，如果你遇到了Bug或者有任何意见反馈，请联系HarukiBot分布式Bot主反馈，HarukiBot分布式Bot主会将问题反馈到Haruki Dev Team
+ 该使用文档或许存在遗漏或者错误内容，如果发现请联系HarukiBot分布式Bot主反馈，HarukiBot分布式Bot主会将问题反馈到Haruki Dev Team
...
+ 从Haruki neo版本起，所有指令都需要带"/"（例"/绑定"），没有"/"的指令不会被响应。

### 服务器支持与切换

+ Harukibot支持日服（jp）、台服（tw）、韩服（kr）、国际服（en）以及国服（cn），使用bot指令时需要带对应服务器前缀（例：查国服个人信息时应使用"/cn个人信息"），无前缀时默认为日服（jp）。
+ 部分功能不支持部分区服，会在功能内说明。
...
<details>
<summary>活动相关的可选参数以及示例可以点击这条展开查看</summary>
