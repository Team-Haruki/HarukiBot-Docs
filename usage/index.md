...
+ 如果您不知道有什么群可以加，也可以访问[Haruki工具箱](https://haruki.seiunx.com)
  的[推荐群聊](https://haruki.seiunx.com/friend_groups)页面。
+ 推荐群聊页面仅接受熟人申请。
+ HarukiBot NEO目前处于公测状态，可能存在各种各样的Bug，如果您遇到了Bug或者有任何意见反馈，请联系HarukiBot分布式Bot主反馈，HarukiBot分布式Bot主会将问题反馈到Haruki
  Dev Team
...
+ 该Bot不提供私聊服务。
+ 使用该Bot，即意味着您同意[使用条款](/licence/)及[隐私条款](/privacy/)。
+ 如果您在使用过程中遇到任何问题，您可以在该页面最下方的``关于``下面联系``开发者``进行反馈。
+ 目前HarukiBot NEO**仍然处于开发状态**，**有多种功能仍在开发中**，如果没有您需要的功能，请不要着急，请耐心等待开发者开发完善。
...
| 指令                                                  | 功能                                                                            |
|:----------------------------------------------------|:------------------------------------------------------------------------------|
| `/绑定` `/pjsk bind` `/pjsk id` `/pjsk 绑定`            | 通过游戏uid绑定您的游戏账号，如"/绑定 114514"。                                                |
...
| `/解绑` `/pjsk unbind` `/取消绑定`                        | 通过游戏uid解绑您的账号，如/解绑 114514。                                                    |
...
| `/pjsk验证` `/pjsk verify`                            | 验证自己的游戏账号，部分功能需要验证后使用。                                                        |
| `/pjsk验证列表` `/pjsk verify list` `/pjsk验证状态`         | 查看已验证的账号列表。                                                                   |
| `/上传个人信息背景` `/pjsk upload profile bg` `/上传个人背景`     | 上传自定义的个人信息背景图，需要消息内含有图片或回复上文中已有的某张图片。                                         |
...
<details>
<summary>乐曲相关的可选参数以及示例可以点击这条展开查看</summary>
...
<details>
<summary>组卡部分的可用参数以及示例 可以点击这条展开查看</summary>
...
| 指定团名+颜色加成    | 任意团 任意颜色                     |                                                                                |
| 指定最低实效要求     | 任意实效                         |                                                                                |
| 指定区域道具配置     | `顶配` `次顶配`                   | 使用与您卡组无关的顶配组卡，顶配区域道具为满级，次顶配为15级。                                               |
...
:::
> ⚠️ **所有 MySekai 指令需用户绑定 Toolbox 账号**
> ⚠️ **本功能不支持国服**
> ⚠️ **台服（tw）需要烤森相册内有照片方能使用ms相关功能**
...
## 关于
+ HarukiBot NEO原实现 - LunaBot: [ルナ茶](https://github.com/NeuraXmy)
+ HarukiBot NEO开发者: [星雲希凪](https://github.com/MejiroRina)、[灵潜](https://github.com/xuanmingLQ)、[Deseer](https://github.com/Deseer)、[storyxy3](https://github.com/storyxy3)
...
+ wiki适配与编辑：[岩崎阳子](https://space.bilibili.com/11048929)、Aposetles、[星雲希凪](https://github.com/MejiroRina)、[storyxy3](https://github.com/storyxy3)
